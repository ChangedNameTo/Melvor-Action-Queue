...
// @version      0.5
// @description  Adds an interface to queue up actions based on triggers you set. Improves the script by 8992.
...
function checkAmmoQty(id) {
  const set = equipmentSets.find(
    (a) => a.equipment[CONSTANTS.equipmentSlot.Quiver] == id
  );
...
function setTrigger(category, name, greaterThan, masteryItem, number) {
  ...
  switch (category) {
    case "Idle":
      return () => {
        return !isInCombat && offline.skill == null;
      };
    case "Item Quantity":
      if (greaterThan == "≥") {
        return () => {
    ...
    case "Equipped Item Quantity":
      ...
      if (
        items
          .filter((a) => a.equipmentSlot == CONSTANTS.equipmentSlot.Summon)
          .find((a) => a.name == name)
      ) {
        return () => {
          let chargeIndex = 1;
          let equipmentSlot = equippedItems.indexOf(itemID);
          if (equipmentSlot === CONSTANTS.equipmentSlot.Summon) chargeIndex = 0;
          let equippedTabletQuantity =
            equipmentSets[selectedEquipmentSet].summonAmmo[chargeIndex];
          return equippedTabletQuantity <= number;
        };
      }
      return () => {
    ...
    case "Enemy in Combat": {
      ...
      return () => {
        return isInCombat && forcedEnemy == monsterID;
      };
    }
  }
}
...
function setAction(actionCategory, actionName, skillItem, skillItem2, qty) {
  ...
  switch (actionCategory) {
      ...
      return () => {
        ...
        for (const area of slayerAreas) {
          if (
            ...
            (area.dungeonCompleted < 0 ||
              dungeonCompleteCount[area.dungeonCompleted] > 0)
          ) {
            selectMonster(monsterIndex);
            return true;
          }
    ...
    case "Switch Equipment Set": {
      ...
      return () => {
        if (playerModifiers.increasedEquipmentSets >= equipSet) {
    ...
    case "Equip Item":
      return () => {
        ...
        if (items[itemID].equipmentSlot == CONSTANTS.equipmentSlot.Summon) {
          equipItem(
            itemID,
            bank[bankID].qty,
    ...
    case "Equip Summon":
      return () => {
        const summonSlot = parseInt(actionName) - 1;
        const bankID = getBankId(itemID);
        if (bankID === -1) return false;
        if (sellItemMode) toggleSellItemMode();
        equipItem(
    ...
    case "Equip Passive":
      return () => {
        if (dungeonCompleteCount[15] < 1 || getBankId(itemID) === -1)
          return false;
        equipItem(itemID, 1, selectedEquipmentSet, false, true);
        return equippedItems[CONSTANTS.equipmentSlot.Passive] === itemID;
      };
    case "Unequip Item": {
      return () => {
    ...
    case "Build Agility Obstacle": {
      ...
      return () => {
        ...
        if (
          !canIAffordThis(
            agilityObstacles[obstacleID].cost,
            agilityObstacles[obstacleID].requirements,
            obstacleID
          )
        )
          return false;
...
function setSkillAction(actionName, skillItem, skillItem2) {
  ...
  switch (actionName) {
    ...
    case "Cooking":
      ...
      return () => {
        if (skillLevel[CONSTANTS.skill.Cooking] < items[itemID].cookingLevel)
          return false;
    ...
    case "Crafting":
      ...
      return () => {
        if (
          ...
          craftingItems[actionID].craftingLevel
        )
          return false;
    ...
    case "Firemaking":
      ...
      return () => {
        if (skillLevel[CONSTANTS.skill.Firemaking] < logsData[actionID].level)
          return false;
    ...
    case "Fishing": {
      ...
      return () => {
        if (
          (!equippedItems.includes(CONSTANTS.item.Barbarian_Gloves) &&
            areaID == 6) ||
          (!secretAreaUnlocked && areaID == 7) ||
          skillLevel[CONSTANTS.skill.Fishing] <
            fishingItems[fishIndex].fishingLevel
        )
          return false;
        if (!isFishing) {
    ...
    case "Fletching":
      ...
      if (skillItem != "Arrow Shafts") {
        return () => {
          if (
            ...
            fletchingItems[actionID].fletchingLevel
          )
            return false;
      ...
      return () => {
        if (
          ...
          return false;
        if (selectedFletchLog != log || selectedFletch !== actionID)
          selectFletch(actionID, log);
        if (!isFletching) startFletching(true);
        return true;
      };
    case "Herblore":
      ...
      return () => {
        if (
          ...
          herbloreItemData[actionID].herbloreLevel
        )
          return false;
      ...
      return () => {
        if (
          skillLevel[CONSTANTS.skill.Magic] <
            ALTMAGIC[actionID].magicLevelRequired ||
          skillLevel[CONSTANTS.skill.Smithing] <
            smithingItems.find((a) => a.smithingLevel == skillItem2)
        )
          return false;
        if (selectedAltMagic !== actionID) selectMagic(actionID);
        if (
          ALTMAGIC[actionID].selectItem >= 0 &&
          selectedMagicItem[ALTMAGIC[actionID].selectItem] !== magicItem
        ) {
          if (
            ALTMAGIC[actionID].selectItem != 0 &&
            (getBankQty(magicItem) < 1 || lockedItems.includes(magicItem))
          )
            return false;
    ...
    case "Runecrafting":
      ...
      return () => {
        if (
          ...
          runecraftingItems[actionID].runecraftingLevel
        )
          return false;
    ...
    case "Smithing":
      ...
      return () => {
        if (
          skillLevel[CONSTANTS.skill.Smithing] <
          smithingItems[actionID].smithingLevel
        )
          return false;
    ...
    case "Thieving":
      ...
      return () => {
        if (skillLevel[CONSTANTS.skill.Thieving] < thievingNPC[actionID].level)
          return false;
      ...
      return () => {
        ...
        treeCuttingHandler.forEach((tree, i) => {
          if (
            tree !== null &&
            !actionID
              .slice(0, playerModifiers.increasedTreeCutLimit + 1)
              .includes(i)
          )
            cutTree(i);
        });
        for (let i = 0; i <= playerModifiers.increasedTreeCutLimit; i++) {
          if (treeCuttingHandler[actionID[i]] === null) {
            if (
              skillLevel[CONSTANTS.skill.Woodcutting] >=
              trees[actionID[i]].level
            ) {
              cutTree(actionID[i]);
            } else {
              result = false;
...
function actionDescription(
  ...
  switch (actionCategory) {
    ...
    case "Equip Item":
      ...
      break;
    case "Equip Summon":
      description += `equip ${actionName} to summon slot`;
      break;
    case "Equip Passive":
...
function submitForm() {
  try {
    ...
    const triggerValid = validateInput(
      options.triggers,
      arr.slice(0, 5).filter((a) => a !== "")
    );
    const actionValid = validateInput(
      options.actions,
      arr.slice(5).filter((a) => a !== "")
    );

    if (triggerValid && actionValid) {
...
function loadAQ() {
  ...
  //add mastery/action names for each skill
  {
    ...
    craftingItems.forEach((item) => {
      options.triggers["Mastery Level"]["Crafting"][items[item.itemID].name] =
        "num";
      options.actions["Start Skill"]["Crafting"][items[item.itemID].name] =
        null;
    });
    ...
    smithingItems.forEach((item) => {
      options.triggers["Mastery Level"]["Smithing"][items[item.itemID].name] =
        "num";
      options.actions["Start Skill"]["Smithing"][items[item.itemID].name] =
        null;
    });
  ...
  //agility obstacles
  options.actions["Build Agility Obstacle"] = agilityObstacles.reduce(
    (obj, a) => {
  ...
  for (const a of items.filter(
    (a) => a.equipmentSlot == CONSTANTS.equipmentSlot.Quiver
  ))
    options.triggers["Equipped Item Quantity"][a.name] = "num";
  for (const a of items.filter(
    (a) => a.equipmentSlot == CONSTANTS.equipmentSlot.Summon
  ))
    options.triggers["Equipped Item Quantity"][a.name] = "num";
  ...
  for (const a of items.filter(
    ...
    options.actions["Equip Summon"][a.name] = { 1: null, 2: null };
